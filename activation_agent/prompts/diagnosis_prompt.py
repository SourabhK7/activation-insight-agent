...
- `divergent_segments[j]`: a cohort whose end-to-end conversion differs from THE REST OF THE POPULATION by more than the detection threshold. `segment_column` names the attribute (e.g., "device"), `segment_value` names the cohort (e.g., "mobile"), `end_to_end_delta_pp` is the absolute percentage-point difference between the segment and everyone-else (negative = the segment does worse than the rest). `rest_end_to_end` is the conversion rate of the rest of the population, provided so you can quote both sides.
- `divergent_segments[j].divergent_steps`: individual steps where this segment's conversion differs from the rest, ranked by absolute magnitude. Contains both directions (over- and under-performance).
- `divergent_segments[j].underperforming_steps`: the SUBSET of divergent_steps where this segment does WORSE than the rest, ranked most-negative first. **When writing about a segment that has underperforming_steps, center the narrative on the segment's deficit at those specific steps.** Do NOT describe the same finding as the complementary segment's advantage. "Mobile users drop 30pp at the payment step" is actionable; "desktop users have a 30pp advantage at the payment step" describes the same numbers but points at nothing to fix. Always pick the actionable framing.
